                                        ...
                                        <div className="flex gap-2 relative">
                                            {updatingId === apt.id ? (
                                                ...
                                                <div className="flex items-center gap-2">
                                                    {['pending', 'confirmed', 'arrived', 'cancelled'].filter(s => s !== apt.status).map(status => (
                                                        <button
                                                            key={status}
                                                            onClick={() => handleStatusUpdate(apt.id, status)}
                                                            className="px-3 py-1.5 text-xs font-bold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-indigo-50 hover:text-indigo-600 dark:hover:bg-indigo-900/30 dark:hover:text-indigo-400 rounded-lg transition-all border border-transparent hover:border-indigo-100 dark:hover:border-indigo-800 capitalize"
                                                        >
                                                            Mark {status}
                                                        </button>
                                                    ))}
